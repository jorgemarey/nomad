...
const (
	envNamespace = "RHO_NAMESPACE"
	envURL       = "RHO_URL"
	envAPIKey    = "RHO_API_KEY"
)
...
// Client represent a client to the rho api
type Client struct {
...
func (c *Client) do(requester requester, data interface{}) error {
	...
	if resp.StatusCode >= 400 {
		rhoErr := &RhoCreateError{}
		err = json.Unmarshal(bb, rhoErr)
		if rhoErr.Code == 0 {
			return fmt.Errorf("Unexpected status code: %d", resp.StatusCode)
		}
		if len(rhoErr.InvalidEntities) == 0 {
			return &rhoErr.rhoError
		}
		return rhoErr
	}
...
// rhoError represents an error ocurring when calling the Rho API
...
// RhoCreateError represents a specific error that has problems on some entities
type RhoCreateError struct {
	rhoError
...
func (e *RhoCreateError) Error() string {
	ie := make([]string, 0)
